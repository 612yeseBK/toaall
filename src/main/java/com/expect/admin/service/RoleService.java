...
public class RoleService {
	...
	/**
	 * 根据流程类别（模块分类）获取所有相关的角色
	 */
	public List<RoleVo> getRolesByLclb(String lclb){
		Set<Role> roles = new HashSet<>();
		Function parentFunction = functionRepository.findByName(lclb);
		if(parentFunction == null)
			return new ArrayList<>();
		Set<Function> childFunction = parentFunction.getChildFunctions();
		if(!CollectionUtils.isEmpty(childFunction)){
			for(Function function : childFunction){
				Set<Role> roles1 = function.getRoles();
