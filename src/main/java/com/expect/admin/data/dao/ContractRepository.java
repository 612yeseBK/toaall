...
public interface ContractRepository extends JpaRepository<Contract, String>{
	...
	public List<Contract> findAllByLcbs(String lcid);
	
	List<Contract> findByHtshztOrderBySqsjDesc(String condition);

	List<Contract> findByLcbsAndHtshztOrderBySqsjDesc(String lcbs, String conditon);
	...
	@Query("select c from Contract c where c.nhtr.id = ?1 and c.htshzt = ?2 or c.htshzt = 'S' order by c.sqsj desc")
	public List<Contract> findSqjlWtjList(String userId, String condition);
	
	@Query("select c from Contract c where c.nhtr.id = ?1 and c.htshzt <> 'T' and c.htshzt <> 'Y' and c.htshzt <> 'S' and c.htshzt <> ?2 order by c.sqsj desc")
...
//	@Query("select distinct c from Contract c, Lcrzb l where c.id = l.clnrid and l.user.id = ?1 and l.cljg = '通过'")
	@Query("select distinct c from Contract c, Lcrzb l where c.id = l.clnrid and l.user.id = ?1 and c.htshzt <> ?2 and c.htshzt <> 'deactivation' and c.htshzt <> 'revocation' order by c.sqsj desc")
